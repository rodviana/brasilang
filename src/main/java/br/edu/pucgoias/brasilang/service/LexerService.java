...
public final class LexerService {
    ...
    private char consumeAndGetCurrentCharacter(Lexer l) {
        ...
        if (c == '\n') { l.setLine(l.getLine() + 1); l.setCol(1); }
        else           { l.setCol(l.getCol() + 1); }
        return c;
    }

    private void consumeCharacters(Lexer l, int count) {
        for (int k = 0; k < count && !isEndOfSource(l); k++) consumeAndGetCurrentCharacter(l);
    }
    ...
    private boolean startsWith(Lexer l, String s) {
        if (isEndOfSource(l)) return false;
        for (int k = 0; k < s.length(); k++) {
            if (preVisualizeNextCharacter(l, k) != s.charAt(k)) return false;
        }
        return true;
    }
    ...
    private boolean skipBlockCommentIfPresent(Lexer l) {
        if (startsWith(l, "/*")) {
            ...
            if (!isEndOfSource(l)) consumeCharacters(l, 2); // "*/"
            return true;
        }
        return false;
    }
    ...
    private Token tryReadDoubleOperator(Lexer l, int startLine, int startCol) {
        ...
        if (c0 == '=' && c1 == '=') { consumeCharacters(l, 2); return t(EnumTokenType.EQ,  "==", startLine, startCol); }
        if (c0 == '!' && c1 == '=') { consumeCharacters(l, 2); return t(EnumTokenType.NEQ, "!=", startLine, startCol); }
        if (c0 == '<' && c1 == '=') { consumeCharacters(l, 2); return t(EnumTokenType.LE,  "<=", startLine, startCol); }
        if (c0 == '>' && c1 == '=') { consumeCharacters(l, 2); return t(EnumTokenType.GE,  ">=", startLine, startCol); }
        return null;
    }
    ...
    private Token tryReadSingleSymbolOrSimpleOperator(Lexer l, int startLine, int startCol) {
        ...
        switch (c0) {
            case '(': consumeCharacters(l, 1); return t(EnumTokenType.LPAR,   "(", startLine, startCol);
            case ')': consumeCharacters(l, 1); return t(EnumTokenType.RPAR,   ")", startLine, startCol);
            case '{': consumeCharacters(l, 1); return t(EnumTokenType.LBRACE, "{", startLine, startCol);
            case '}': consumeCharacters(l, 1); return t(EnumTokenType.RBRACE, "}", startLine, startCol);
            case ':': consumeCharacters(l, 1); return t(EnumTokenType.COLON,  ":", startLine, startCol);
            case ';': consumeCharacters(l, 1); return t(EnumTokenType.SEMI,   ";", startLine, startCol);
            case ',': consumeCharacters(l, 1); return t(EnumTokenType.COMMA,  ",", startLine, startCol);
            case '+': consumeCharacters(l, 1); return t(EnumTokenType.PLUS,   "+", startLine, startCol);
            case '-': consumeCharacters(l, 1); return t(EnumTokenType.MINUS,  "-", startLine, startCol);
            case '*': consumeCharacters(l, 1); return t(EnumTokenType.STAR,   "*", startLine, startCol);
            case '/': consumeCharacters(l, 1); return t(EnumTokenType.SLASH,  "/", startLine, startCol);
            case '=': consumeCharacters(l, 1); return t(EnumTokenType.ASSIGN, "=", startLine, startCol);
            case '<': consumeCharacters(l, 1); return t(EnumTokenType.LT,     "<", startLine, startCol);
            case '>': consumeCharacters(l, 1); return t(EnumTokenType.GT,     ">", startLine, startCol);
            default:  return null;
        }
    }
    ...
    private Token tryReadNumber(Lexer l, int startLine, int startCol) {
        ...
        if (!(Character.isDigit(c0) || (c0 == '.' && Character.isDigit(c1)))) return null;
    ...
    private Token tryReadStringLiteral(Lexer l, int startLine, int startCol) {
        if (preVisualizeNextCharacter(l, 0) != '"') return null;
    ...
    private static Token t(EnumTokenType tt, String lex, int l, int c) { return new Token(tt, lex, l, c); }
